...
class GraphBuilder:
    ...
    def __make_subsentences(self, spacy_toks, action_tags):
        ...
        op_id = 0
        next_op = action_ids[op_id]
        for op, next_op in zip(action_ids[:-1], action_ids[1:]):
            # if action_tags[op] != "":
            sub_sentences.append(spacy_toks[op:next_op])
        sub_sentences.append(spacy_toks[next_op:])
        # op_id = 0
        # next_op = action_toks[op_id][0].i
        # for op_id in range(len(action_toks[:-1])):
        #     op = action_toks[op_id][0].i
        #     next_op = action_toks[op_id + 1][0].i
        #     sub_sentences.append(raw_tokens[op:next_op])
        # sub_sentences.append(raw_tokens[next_op:])

        return sub_sentences

    def __get_subjects_list(self, current_op, spacy_tokens, action_tags):
    ...
    def build_graph(self, sentence_tokens, action_tags, materials=[]):
        ...
        action_seq = [(i, op) for i, op in enumerate(action_tags) if op != ""]
        if action_seq:
        ...
        # action_seq_tok = [(spacy_tokens[i], op) for i, op in action_seq]
        action_seq_tok = [(t, action_tags[t.i]) for t in spacy_tokens if
        ...
        prev_subjects_list = []
        prev_op_tok = ""
        for num_op, ((op_tok, op_type), sub_sent) in enumerate(zip(action_seq_tok, sub_sentences)):
            temp_toks, time_toks = [], []
            env_ids = [[], []]
            env_toks = ["", ""]
            ...
            # Assuming that action subject is located in the sentence chunk starting from previous action/verb
            # and spanning till current action
            subjects_list = self.__get_subjects_list(op_tok, spacy_tokens, action_tags)
            ...
            # if action refer to previous step reference_op = True, else reference_op = True
            ...
            if self.__verbose:
                ...
                print("\tReference action:", reference_op)

            # Finding mixing conditions and type
            # Mixing types: solid mix, solution mix, mix with liquid
            if op_type in ["Mixing", "Heating", "Cooling"] or any(w in op_tok.text for w in ["dry", "evaporate"]):
            ...
            # if op_type in ["DryingAction", "QuenchingAction"]:
            #     temp_toks = [t for t in get_temperatures_toks(sub_sent_text)]
